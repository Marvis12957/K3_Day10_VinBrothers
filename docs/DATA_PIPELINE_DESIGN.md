...
flowchart TD
    subgraph PHA1["PHA 1 — Baseline (dữ liệu sạch)"]
        ...
        B --> R1["data/raw/crossref_response.json<br/>payload gốc — nguồn parse lại"]
        B --> R2["data/raw/crossref_records.json<br/>24 PaperRecord — nguồn repair pha 2"]
    ...
    subgraph PHA2["PHA 2 — Corruption / Repair / Comparison"]
        D1 --> CO["src/ingestion/corruption.py<br/>7 kịch bản lỗi + rebuild text, có log"]
        ...
        R2 --> RC["load_raw_records + build_clean_dataframe<br/>REPAIR từ raw snapshot"]
